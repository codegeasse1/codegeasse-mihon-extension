...
class TheBlank : HttpSource(), ConfigurableSource {
    ...
    private fun apiRequest(
        ...
        if ((includeXSRFToken && xsrfToken == null) || (includeCSRFToken && csrfToken == null) || (includeVersion && version == null)) {
            val document = client.newCall(GET(baseHttpUrl, headers)).execute().also {
                if (!it.isSuccessful) {
                    it.close()
            ...
            version = json.decodeFromString<Version>(document.selectFirst("#app")!!.attr("data-page")).version
            csrfToken = document.selectFirst("meta[name=csrf-token]")!!.attr("content")
            xsrfToken = client.cookieJar.loadForRequest(baseHttpUrl).first { it.name == "XSRF-TOKEN" }.value
        }
        ...
        val apiHeaders = headersBuilder().apply {
            ...
            if (includeXSRFToken) set("X-XSRF-TOKEN", xsrfToken!!)
            if (includeCSRFToken) set("X-CSRF-TOKEN", csrfToken!!)
        }.build()

        return if (body != null) POST(url.toString(), apiHeaders, body) else GET(url, apiHeaders)
    }

    // ============================== Search & Browse ==============================
    private val popularFilters = FilterList(SortFilter("Sort", sortValues, Filter.Sort.Selection(3, false)))
    private val latestFilters = FilterList(SortFilter("Sort", sortValues, Filter.Sort.Selection(2, false)))
    ...
    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request {
        ...
        val url = baseHttpUrl.newBuilder().apply {
            ...
            filters.filterIsInstance<CheckBoxGroup>().firstOrNull()?.also { status ->
                if (status.checked.isNotEmpty()) addQueryParameter("status", status.checked.joinToString(","))
            }
            filters.filterIsInstance<SortFilter>().firstOrNull()?.also { sort ->
                addQueryParameter("orderby", sort.sort)
                if (sort.ascending) addQueryParameter("order", "asc")
            }
        }.build()
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        ...
        if (response.request.url.queryParameter("q") != null) {
            val data = json.decodeFromString<List<BrowseManga>>(bodyStr)
            return MangasPage(mangas = data.map { it.toSManga(::createThumbnailUrl) }, hasNextPage = false)
        } else {
            val data = json.decodeFromString<LibraryResponse>(bodyStr).series
            return MangasPage(
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        return SManga.create().apply {
            ...
            description = buildString {
                data.description?.also { append(it.trim(), "\n\n") }
                data.releaseYear?.also { append("Sortie: ", it, "\n\n") }
                data.alternativeName?.also { append("Noms alternatifs: ", it) }
            }.trim()
            genre = buildList {
    ...
    // ============================== Pages & Encryption ==============================
    override fun pageListRequest(chapter: SChapter): Request = apiRequest("$baseUrl${chapter.url}".toHttpUrl(), includeXSRFToken = true, includeCSRFToken = false, includeVersion = true)
    ...
    private fun ensureSession(serieSlug: String, chapterSlug: String): ChapterSession {
        ...
        val lock = sessionLocks[id] ?: Any().let { fresh -> sessionLocks.putIfAbsent(id, fresh) ?: fresh }
        synchronized(lock) {
            sessions[id]?.let { return it }

            val url = "$baseUrl/serie/$serieSlug/chapter/$chapterSlug".toHttpUrl()
            val req = apiRequest(url, includeXSRFToken = true, includeCSRFToken = false, includeVersion = true)
            val props = client.newCall(req).execute().use { resp ->
                if (!resp.isSuccessful) throw IOException("Could not rebuild chapter session: HTTP ${resp.code}")
                json.decodeFromString<PageListResponse>(resp.body!!.string()).props
            }
    ...
    override fun pageListParse(response: Response): List<Page> {
        val props = json.decodeFromString<PageListResponse>(response.body!!.string()).props
        val sess = handshakeFrom(props)
    ...
    private fun imageInterceptor(chain: Interceptor.Chain): Response {
        ...
        if (sessionId == THUMBNAIL_FRAGMENT) return response
    ...
    private class TriStateFilter(name: String, val value: String) : Filter.TriState(name)
    ...
    abstract class TriStateGroupFilter(name: String, options: List<Pair<String, String>>) : Filter.Group<TriStateFilter>(name, options.map { TriStateFilter(it.first, it.second) }) {
        val included get() = state.filter { it.isIncluded() }.map { it.value }
        val excluded get() = state.filter { it.isExcluded() }.map { it.value }
    }

    abstract class CheckBoxGroup(name: String, options: List<Pair<String, String>>) : Filter.Group<CheckBoxFilter>(name, options.map { CheckBoxFilter(it.first, it.second) }) {
        val checked get() = state.filter { it.state }.map { it.value }
    }

    class SortFilter(name: String, private val sortValues: List<Pair<String, String>>, selection: Selection = Selection(0, false)) : Filter.Sort(name = name, values = sortValues.map { it.first }.toTypedArray(), state = selection) {
    ...
    companion object {
        ...
        private val sortValues = listOf("New Series" to "date", "Trending" to "trending", "Recently Updated" to "recently", "Most Views" to "views", "A-Z" to "alphabetical")
        private val genres = listOf("Action" to "action", "Adventure" to "adventure", "Ai" to "ai", "Animated" to "animated", "Anthology" to "anthology", "Cohabitation" to "cohabitation", "College" to "college", "Comedy" to "comedy", "Doujinshi" to "doujinshi", "Drama" to "drama", "Fantasy" to "fantasy", "Folklore" to "folklore", "Harem" to "harem", "Historical" to "historical", "Horror" to "horror", "Isekai" to "isekai", "Josei" to "josei", "Love triangle" to "love-triangle", "Martial arts" to "martial-arts", "Mature" to "mature", "Murim" to "murim", "Mystery" to "mystery", "Office workers" to "office-workers", "Psychological" to "psychological", "Robots" to "robots", "Romance" to "romance", "School life" to "school-life", "Sci-fi" to "sci-fi", "Seinen" to "seinen", "Shoujo" to "shoujo", "Shounen" to "shounen", "Slice of life" to "slice-of-life", "Smut" to "smut", "Sports" to "sports", "Supernatural" to "supernatural", "Superpower" to "superpower", "System" to "system", "Thriller" to "thriller", "Uncensored" to "uncensored", "Violence" to "violence", "Workplace" to "workplace")
        private val type = listOf("Comic" to "comic", "Doujin" to "doujin", "Josei" to "josei", "Manga" to "manga", "Manhua" to "manhua", "Manhwa" to "manhwa", "Pornhwa" to "pornhwa", "Webtoon" to "webtoon")
        private val status = listOf("Ongoing" to "ongoing", "Finished" to "finished", "Dropped" to "dropped", "On Hold" to "onhold", "Upcoming" to "upcoming")
    }
}

// ============================== Data Transfer Objects ==============================
@Serializable class Version(val version: String)

@Serializable class LibraryResponse(val series: Series) {
    @Serializable class Series(val data: List<BrowseManga>, val meta: Meta) {
        @Serializable class Meta(@SerialName("current_page") val current: Int, @SerialName("last_page") val last: Int)
    }
}

@Serializable class BrowseManga(private val slug: String, @JsonNames("name") private val title: String, @JsonNames("cover_image") private val image: String? = null) {
...
@Serializable class MangaResponse(val props: Props) {
    @Serializable class Props(val serie: Manga) {
        @Serializable class Manga(
            val slug: String,
            @JsonNames("name") val title: String,
            @JsonNames("cover_image") val image: String? = null,
            val description: String? = null,
            val author: String? = null,
            val artist: String? = null,
            @SerialName("name_alternative") val alternativeName: String? = null,
        ...
        @Serializable class Chapter(val slug: String, val title: String, val createdAt: String, val isPremium: Boolean)
        @Serializable class Name(val name: String)
    }
}

@Serializable class PageListResponse(val props: Props) {
    @Serializable class Props(
        ...
        @Serializable class Data(val slug: String, val serie: Serie)
        @Serializable class Serie(val slug: String)
    }
}
