...
/*
 * GalaxyManga (https://galaxymanga.io) — MangaThemesia theme (WordPress).
 * Shoujo/romance aggregator.
 *
 *     Popular : /manga/?title=&page=N&order=popular
 *     Latest  : /manga/?title=&page=N&order=update
 *     Search  : /manga/?title=<q>&page=N
 *     Manga   : /manga/<slug>                     -> h1.entry-title, .thumb img,
 *               .tsinfo .imptdt rows (Status), .mgen a genres,
 *               .entry-content-single description
 *     Chapters: .eplister li .eph-num a           -> chapter URL /<slug>-chapter-<n>/
 *     Chapter : /<slug>-chapter-<n>/              -> div#readerarea img, else the
 *               page embeds a reader config JSON with "images": [url, ...]
 */
class GalaxyManga : HttpSource() {

    override val name = "GalaxyManga"
    override val baseUrl = "https://galaxymanga.io"
    ...
    override fun popularMangaRequest(page: Int): Request =
        GET("$baseUrl/manga/?title=&page=$page&order=popular", headers)

    override fun popularMangaParse(response: Response): MangasPage {
        val document = response.asDocument()
        val mangas = document.select(".listupd .bs .bsx").mapNotNull(::mangaFromCard)
        val hasNextPage = document.selectFirst(".hpage .r") != null
        return MangasPage(mangas, hasNextPage)
    }

    override fun latestUpdatesRequest(page: Int): Request =
        GET("$baseUrl/manga/?title=&page=$page&order=update", headers)

    override fun latestUpdatesParse(response: Response): MangasPage =
        popularMangaParse(response)

    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request =
        GET("$baseUrl/manga/?title=${URLEncoder.encode(query, "utf-8")}&page=$page", headers)
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        return SManga.create().apply {
            ...
            document.selectFirst(".thumb img, .bigcover img")?.let { thumbnail_url = imageFromElement(it) ?: "" }
            val statusText = document.select(".tsinfo .imptdt").firstOrNull { item ->
                item.text().contains("Status", true)
            }?.text()
            status = when {
            ...
            document.select(".mgen a").eachText().joinToString().takeIf { it.isNotBlank() }?.let { genre = it }
