 ...
 *
 *     Popular : /?page=N&sort=popular     (cards: a[href*='/manga/'], cover img imgs/cover/...)
 *     Latest  : /?page=N
 *     Search  : /?search=<query>
 *     Detail  : /manga/<slug>.<id>        (h1, cover imgs/cover/..., chapters table div.chapter a[href*='/c'])
...
class MangaKatana : HttpSource() {
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        val doc = Jsoup.parse(response.body?.string() ?: "", response.request.url.toString())
        return doc.select("div.chapter a[href*='/c'], table.table a[href*='/c'], .chapter a[href*='/c']").mapNotNull { element ->
    ...
    override fun pageListParse(response: Response): List<Page> {
        ...
        val urls = Regex("""'([^']+)'""").findAll(match.groupValues[1])
            .map { it.groupValues[1].trim() }.filter { it.toHttpUrl() != null }.toList()
