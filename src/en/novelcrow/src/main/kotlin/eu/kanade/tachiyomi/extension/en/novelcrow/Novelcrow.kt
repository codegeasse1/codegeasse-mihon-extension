...
/*
 * NovelCrow (https://novelcrow.com) — a Madara WordPress reader behind
 * Cloudflare. Plain server-rendered HTML, chapter images on the same host.
 *
 *     Home/Latest : /comic-genre/hentai/ (+ /page/N/)   -> .page-item-detail grid
 *     Browse      : /comic-genre/<slug>/ (+ /page/N/)   -> .page-item-detail grid
 *     Search      : /?s=<q>&post_type=wp-manga          -> .page-item-detail / .c-tabs-item__content
 *     Details     : /comic/<slug>
 *     Chapters    : li.wp-manga-chapter <a href>  (newest first, relative dates)
 *     Pages       : /comic/<slug>/<n>-<chapter-slug>/ -> .reading-content img
 *
 * Pagination is path-based (/page/N/), detected via a .nextpostslink anchor.
 */
class Novelcrow : HttpSource() {

    override val name = "NovelCrow"
    override val baseUrl = "https://novelcrow.com"
    ...
    override fun popularMangaRequest(page: Int): Request =
        GET("$baseUrl/comic-genre/hentai${pageSuffix(page)}", headers)
    ...
    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request {
        ...
        return if (query.isBlank() && genre != null) {
            ...
            GET("$baseUrl/?s=$q&post_type=wp-manga", headers)
        }
    }

    override fun searchMangaParse(response: Response): MangasPage {
        val isGenre = response.request.url.toString().contains("/comic-genre/")
        val page = parseListing(response)
        // Text-search pagination is unverified on the site — keep it single-page.
        return if (isGenre) page else MangasPage(page.mangas, hasNextPage = false)
    }
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        val title = doc.selectFirst(".post-title h1")?.text()?.trim()
            ?: doc.title().substringBefore(" - Novel Crow").trim()
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        return items.mapIndexed { index, li ->
            ...
            SChapter.create().apply {
                url = a.absUrl("href")
                name = a.text().trim()
                date_upload = parseChapterDate(li.selectFirst(".chapter-release-date"))
    ...
    companion object {
        ...
        // Slugs from the site's genre menu (verified: hentai; others standard Madara slugs).
...
private fun parseChapterDate(el: Element?): Long {
    if (el == null) return 0L
    val text = el.text().trim()
    val title = el.selectFirst("a[title]")?.attr("title")?.trim().orEmpty()
    return parseDate(text.ifEmpty { title })
}
...
private fun parseChapterNumber(name: String): Float? =
    Regex("""chapter\s*([\d.,]+)""", RegexOption.IGNORE_CASE)
        .find(name)?.groupValues?.get(1)?.replace(",", ".")?.toFloatOrNull()
