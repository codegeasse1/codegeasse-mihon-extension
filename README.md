...
## ð¥ Add this repo to your reader

Works with **Mihon**, **Aniyomi**, **Tachiyomi**, **Nekoread** and any other reader that loads Mihon/Tachiyomi-style extensions.

1. Open your reader.
2. Go to **Settings â Extensions** (in Mihon/Aniyomi: open **Browse â Extensions**, tap the **â® (three-dot) menu** in the top right, then **Extension repos**).
...
4. Back in the Extensions list the **Codegeasse** repo appears â install whichever sources you want.

Everything installs as normal Mihon extensions and auto-updates whenever the repo is refreshed in the app.

## ð Extensions in this repo
...
> The index also carries a placeholder **Example** extension (`eu.kanade.tachiyomi.extension.en.example`) used for testing â safe to ignore.
...
## ð  How this repo works

- Each extension is a standalone module written and maintained in this repository.
- Every push to `main` is compiled by GitHub Actions. The workflow builds each extension module and publishes the extension index (`index.json`), the APKs (`apk/`) and icons (`icon/`) to the **`repo`** branch â that's what your reader fetches when you add the repository above.
- Want a new site? Open an issue and it may be added.

## ð©âð» For developers
...
All manga content is hosted on the source websites listed above â this repo only provides the extension code that connects to and reads those already-public sites. We don't host, upload, or store any content, and we don't own any of the source websites. All titles, logos and content belong to their respective owners.
